...
// Explicit Types to clear up TS warnings
interface Tier {
  min_qty: number;
  price: string;
}
...
export default function CartPage() {
  ...
  const [platformCharge] = useState<number>(80); // Set as constant since it doesn't change dynamically
  ...
  // CALCULATIONS (Fixed to base minimum order on product subtotal values)
  ...
  const grandTotal = Math.round(
    subtotal + transportCharge + handlingFees
  );
  
  // Validation should check subtotal value, not subtotal + charges
