                ...
                <div className="bg-white p-2 rounded-2xl shadow-md border border-red-50 flex flex-wrap gap-1">
                    {(['all', 'pending', 'approved', 'rejected'] as const).map((s) => (
                        <button
                            key={s}
                            onClick={() => setFilter(s)}
                            className={`px-6 py-2.5 rounded-xl text-sm font-black transition-all flex items-center gap-3 ${
                                filter === s 
                        ...
                        >
                            ...
                            <span className={`px-2 py-0.5 rounded-md text-[10px] ${
                                filter === s ? 'bg-white/20 text-white' : 'bg-red-50 text-red-600'
                            }`}>
                                {count(s)}
                            </span>
                        </button>
                                ...
                                <tr key={user.id} className="hover:bg-red-50/30 transition-colors group">
                                    ...
                                    <td className="px-8 py-6 text-right">
                                        <div className="flex justify-end gap-2">
                                            ...
                                            {user.status === 'pending' && (
                                                <>
                                                    <button onClick={() => handleStatusUpdate(user.id, 'approved')} className="p-3 bg-white border border-red-100 text-red-600 rounded-2xl hover:bg-red-600 hover:text-white transition-all shadow-sm">
                ...
                <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex justify-end transition-all">
                    <div className="w-full max-w-xl bg-white h-full shadow-2xl p-0 flex flex-col animate-in slide-in-from-right duration-300">
                        ...
                        {selectedUser.status === 'pending' && (
                            <div className="p-8 bg-white border-t border-red-50 grid grid-cols-2 gap-4">
                                <button onClick={() => handleStatusUpdate(selectedUser.id, 'rejected')} className="py-4 rounded-2xl font-black text-slate-400 border-2 border-slate-100 hover:bg-slate-50 transition-all">Reject</button>
                                <button onClick={() => handleStatusUpdate(selectedUser.id, 'approved')} className="py-4 rounded-2xl font-black bg-red-600 text-white shadow-lg shadow-red-100 hover:bg-red-700 transition-all">Approve Partner</button>
                            </div>
                        )}
                    </div>
                </div>
